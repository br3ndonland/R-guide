...
```{r code-ttest-atheroma-01-import, message=FALSE}
...
```{r code-ttest-hsp70-01-import, message=FALSE}
...
`aztcpm` dataset from University of Illinois statistics course [PATH 591 lab 02](https://github.com/br3ndonland/R-guide/tree/master/lfs/PATH591-Lab-02.url) and [PATH 591 lecture 04 A](https://github.com/br3ndonland/R-guide/tree/master/lfs/PATH591-Lecture-04A-annotated.url)
...
```{r code-lm-simple-aztcpm-01-import, message=FALSE}
...
```{r code-lm-simple-jfs-feedtc-01-import, message=FALSE}
...
```{r code-lm-simple-beer-01-import, message=FALSE}
...
```{r code-anova-adipose-weights-01-import, message=FALSE}
adipose_weights <-
...
```{r code-anova-adipose-weights-02-run-multcomp, message=FALSE}
...
```{r code-anova-2way-fall2009thickness-01-load, message=FALSE}
fall2009thickness <-
  read.table(
...
```{r code-ancova-litter-01-load, message=FALSE}
library(multcomp)
attach(litter)
table(dose)
...
```{r code-manova-hr-01-read, message=FALSE}
...
```{r code-kruskal-cardiacmg-01-load, message=FALSE}
...
```{r code-kruskal-cardiacmg-03-assumptions-hov, message=FALSE}
library(lawstat)
...
```{r code-plot-volcano-ggd-01-load, message=FALSE}
...
```{r code-plot-volcano-nrf1-01-import, message=FALSE}
proteomics_data <-
  read.table(
